...
addSection('SERVER');
addRow('sv','\uC11C\uBC84 \uC800\uC7A5');
var MAX_ATTEMPTS=3,REQUEST_TIMEOUT=15000;
function sleep(ms){return new Promise(function(res){setTimeout(res,ms);});}
function fetchWithTimeout(url,opt){var ac=new AbortController(),timer=setTimeout(function(){ac.abort();},REQUEST_TIMEOUT);return fetch(url,Object.assign({},opt||{},{signal:ac.signal})).finally(function(){clearTimeout(timer);});}
function xf(id,url,opt,attempt){return fetchWithTimeout(url).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.text();}).then(function(t){var info=t.length>0?(t.length>1024?(t.length/1024).toFixed(1)+'KB':t.length+'B'):'\uC5C6\uC74C';okRow(id,info);console.log('[carol]',id,url,t.length);return t;}).catch(function(e){if(attempt<MAX_ATTEMPTS){setRow(id,'\u21BB','#facc15','\uC7AC\uC2DC\uB3C4');}else if(opt){skipRow(id,'\uC2E4\uD328');}else{failRow(id,e&&e.name==='AbortError'?'\uC2DC\uAC04 \uCD08\uACFC':'\uB124\uD2B8\uC6CC\uD06C \uC624\uB958');}return '';});}
...
async function collectCore(){for(var attempt=1;attempt<=MAX_ATTEMPTS;attempt++){if(attempt>1){setRow('sv','\u21BB','#facc15','\uC7AC\uC2DC\uB3C4 '+attempt+'/'+MAX_ATTEMPTS);}var _pl=[{i:'hm',u:'/maimai-mobile/home/',o:false},{i:'pd',u:'/maimai-mobile/playerData/',o:false},{i:'rc',u:'/maimai-mobile/record/',o:false},{i:'fc',u:'/maimai-mobile/friend/userFriendCode/',o:false},{i:'tb4',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=4',o:true},{i:'tb3',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=3',o:true},{i:'tb2',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=2',o:true},{i:'tb1',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=1',o:true},{i:'tb0',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=0',o:true}];if(server==='intl')_pl.push({i:'rt',u:'/maimai-mobile/home/ratingTargetMusic/',o:true});_pl.push({i:'mp',u:'/maimai-mobile/map/',o:true},{i:'em',u:'/maimai-mobile/map/eventMap/',o:true});var _r={};for(var _pi=0;_pi<_pl.length;_pi++){var _p=_pl[_pi];_r[_p.i]=await xf(_p.i,_p.u,_p.o,attempt);if(_pi<_pl.length-1)await sleep(800);}h=_r.hm;p=_r.pd;rd=_r.rc;f=_r.fc;tb4=_r.tb4;tb3=_r.tb3;tb2=_r.tb2;tb1=_r.tb1;tb0=_r.tb0;rt=_r.rt||'';m=_r.mp;em=_r.em;var required={hm:h,pd:p,rc:rd,fc:f,tb4:tb4,tb3:tb3,tb2:tb2,tb1:tb1,tb0:tb0};var missing=Object.keys(required).filter(function(id){return !required[id];});if(missing.length===0)return validateCore();console.warn('[carol] missing pages attempt',attempt,missing);if(attempt<MAX_ATTEMPTS){missing.forEach(function(id){setRow(id,'\u21BB','#facc15','\uC7AC\uC2DC\uB3C4');});await sleep(500*attempt);}else{missing.forEach(function(id){failRow(id,'\uC218\uC9D1 \uC2E4\uD328');});}}return false;}
async function collectDetails(){var reqs=collectDetailRequests(rd),out=[];for(var i=0;i<reqs.length;i++){var req=reqs[i];for(var attempt=1;attempt<=MAX_ATTEMPTS;attempt++){try{var r=await fetchWithTimeout(req.url);if(!r.ok)throw new Error('HTTP '+r.status);var html=await r.text();out.push({idx:req.idx,html:html});console.log('[carol] detail',req.idx,req.url,html.length);break;}catch(e){if(attempt<MAX_ATTEMPTS)await sleep(400*attempt);else console.warn('[carol] detail fetch failed',req.idx,req.url,e&&e.message?e.message:String(e));}}}return out;}
async function postSync(){for(var attempt=1;attempt<=MAX_ATTEMPTS;attempt++){try{var payload=JSON.stringify({server:server,h:h,p:p,r:rd,f:f,a:a,js:js,dt:dt,tb4:tb4,tb3:tb3,tb2:tb2,tb1:tb1,tb0:tb0,rt:rt,m:m,em:em}),body=payload,headers={'Content-Type':'application/json'};if(typeof CompressionStream!=='undefined'){try{body=await new Response(new Blob([payload]).stream().pipeThrough(new CompressionStream('gzip'))).arrayBuffer();headers['Content-Encoding']='gzip';}catch(_ce){body=payload;}}var resp=await fetchWithTimeout(v+'/sync?code='+c,{method:'POST',headers:headers,body:body});if(resp.ok){svText=await resp.text();if(svText==='no_change'){skipRow('sv','\uC774\uBBF8 \uCD5C\uC2E0 \uC0C1\uD0DC');}else{okRow('sv');}return true;}var errText=await resp.text();if(attempt<MAX_ATTEMPTS){setRow('sv','\u21BB','#facc15','\uC7AC\uC2DC\uB3C4 '+(attempt+1)+'/'+MAX_ATTEMPTS);await sleep(500*attempt);}else{failRow('sv','HTTP '+resp.status+(errText?': '+errText.slice(0,40):''));}}catch(e4){if(attempt<MAX_ATTEMPTS){setRow('sv','\u21BB','#facc15','\uC7AC\uC2DC\uB3C4 '+(attempt+1)+'/'+MAX_ATTEMPTS);await sleep(500*attempt);}else{failRow('sv',e4&&e4.name==='AbortError'?'\uC11C\uBC84 \uC751\uB2F5 \uC2DC\uAC04 \uCD08\uACFC':'\uC5F0\uACB0 \uC2E4\uD328');}}}return false;}
