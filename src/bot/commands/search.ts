...
export const data = new SlashCommandBuilder()
  .setName("검색")
  .setDescription("내 클리어 기록에서 곡을 검색 (생략 시 본인)")
  .addStringOption((opt) =>
    opt.setName("title").setDescription("검색할 곡명 (일부 일치)").setRequired(true).setMaxLength(50),
  )
  .addUserOption((opt) =>
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    const result = await searchResultEmbeds(cached, userId, query, 0);
    await interaction.editReply(result);
  } catch (e) {
    console.error("[검색]", e);
    await interaction.editReply({ content: "검색에 실패했습니다." }).catch(() => {});
  }
}
